...
export async function getSections(extensionView: SideBarView): Promise<WebElement[]> {
  ...
  const sections = await extensionView.findElements(By.className("split-view-view"));
  return sections;
}

export async function runManageCollaborators(sections: WebElement[]): Promise<void> {
  const driver = VSBrowser.instance.driver;
