    ...
    return await RetryHandler.retry(async () => {
      ...
      for (const view of views) {
        ...
        if (title === Extension.displayName || title === "") {
          ...
          const welcomeView = await sidebar.findElement(By.className("welcome-view-content"));
          if (welcomeView) {
          ...
          const sections = await sidebar.findElements(By.className("split-view-view"));

          for (const section of sections) {
            const sectionTitle = await section.findElement(By.css(".title")).getText();
            const sectionText = await section.findElement(By.css(".pane-body")).getText();
...
export async function waitForTerminal(title: string, message?: string): Promise<void> {
  const driver = VSBrowser.instance.driver;
  await driver.wait(async () => {
    return await RetryHandler.retry(async () => {
...
export async function openExistingProject(folder: string): Promise<void> {
  ...
  await RetryHandler.retry(async () => {
    ...
    await VSBrowser.instance.takeScreenshot(getScreenshotName("openExistingProject"));
  });
  await VSBrowser.instance.waitForWorkbench();
...
export async function startDebugging(item = "Debug (Chrome)"): Promise<boolean> {
...
export async function execCommandIfExist(commandName: string, timeout?: number): Promise<void> {
  const driver = VSBrowser.instance.driver;
  await VSBrowser.instance.waitForWorkbench();
  if (os.type() === "Darwin") {
    // command + P
  ...
  const input = await driver.findElement(By.css(".quick-input-and-message .input"));
  await input.sendKeys(commandName);
  await driver.sleep(Timeout.input);
  const lists = (await driver.findElements(By.css(".quick-input-list .monaco-list-row"))) ?? [];
...
async function inputFolderPath(driver: WebDriver, input: InputBox, folder: string) {
  while (true) {
...
async function setInputTextWsl(driver: WebDriver, input: InputBox, path: string): Promise<boolean> {
...
export async function createNewProject(
  ...
  await execCommandIfExist(CommandPaletteCommands.CreateProjectCommand, Timeout.webView);
  ...
  switch (option) {
    ...
    case "spfxreact": {
      ...
      await input.selectQuickPick(CreateProjectQuestion.SpfxSharepointFrameworkInTtk);
      await driver.sleep(Timeout.input);
    ...
    case "importaddin": {
      const importPath: string = testRootFolder + "\\..\\src\\ui-test\\treeview\\office-xml-addin";
      await input.selectQuickPick(CreateProjectQuestion.OfficeAddin);
  ...
  const projectCopyPath = path.resolve(testRootFolder, appName + appNameCopySuffix);
...
export async function setInsiderPreview(): Promise<void> {
  await setExtensionSetting(Extension.settingsCategory, Extension.settingsInsiderPreview, true);
  await execCommandIfExist("Developer: Reload Window", Timeout.webView);
}

export async function resetInsiderPreview(): Promise<void> {
  await setExtensionSetting(Extension.settingsCategory, Extension.settingsInsiderPreview, false);
  await execCommandIfExist("Developer: Reload Window", Timeout.webView);
}

export async function runDeployAadAppManifest(env = "dev"): Promise<void> {
  await execCommandIfExist(CommandPaletteCommands.DeployAadAppManifestCommand, Timeout.webView);
  const driver = VSBrowser.instance.driver;
  await driver.sleep(Timeout.longTimeWait);
  const input = await InputBox.create();
...
export async function validateNotification(text: string): Promise<void> {
  ...
  try {
    ...
    await VSBrowser.instance.takeScreenshot(getScreenshotName("upgradeNotification"));
...
export async function findWordFromTerminal(word: string): Promise<boolean> {
  ...
  const searchBoxs = await VSBrowser.instance.driver.findElements(By.css(".simple-find-part"));
  for (const searchBox of searchBoxs) {
    ...
    result = await (await searchBox.findElement(By.className("matchesCount"))).getText();
    if (result.includes("No results")) {
      ...
      await VSBrowser.instance.takeScreenshot(getScreenshotName("debug failed"));
...
export async function getOutPutError(): Promise<void> {
  ...
  for (const notification of notifications) {
    ...
    if (
      ...
      await VSBrowser.instance.takeScreenshot(getScreenshotName("output error"));
