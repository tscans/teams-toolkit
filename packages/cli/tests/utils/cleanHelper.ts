...
export class GraphApiCleanHelper extends CleanHelper {
  ...
  public static async create(
    ...
    const token = await this.getUserToken(tenantId, clientId, username, password);
  ...
  private static async getUserToken(
    ...
    const credential = await pca.acquireTokenByUsernamePassword(usernamePasswordRequest);
  ...
  public async getAadObjectId(applicationId: string): Promise<string | undefined> {
    const result: any[] = [];
    const response = await this.execute("get", `/applications`, undefined);
    if (response?.data?.value) {
      result.push(...(response?.data?.value as any[]));
  ...
  public async getAad(applicationObjectId: string): Promise<any | undefined> {
    const response = await this.execute("get", `/applications/${applicationObjectId}`, undefined);
    return response?.data;
  }

  public async deleteAad(objectId: string): Promise<void> {
    await this.execute("delete", `/applications/${objectId}`, undefined);
    await this.execute("delete", `/directory/deletedItems/${objectId}`, undefined);
  }
  ...
  public async getInstalledTeamsAppId(userId: string, displayName: string): Promise<string> {
    const response = await this.execute(
      "get",
...
export class SharePointApiCleanHelper extends CleanHelper {
  ...
  public static async create(
    ...
    const token = await this.getSharePointUserToken(tenantId, clientId, username, password);
...
export class AppStudioCleanHelper extends CleanHelper {
  ...
  public static async create(
    ...
    const token = await this.getAppStudioUserToken(tenantId, clientId, username, password);
...
export class DevTunnelCleanHelper {
  ...
  constructor(token: string) {
    this.tunnelManagementClientImpl = new TunnelManagementHttpClient("Teams-Toolkit-UI-TEST", () =>
...
export async function createResourceGroup(appName: string, envName?: string): Promise<boolean> {
  if (!appName) {
    return false;
  }
  const name = `${appName}-${envName}-rg`;
...
export async function createResourceGroupByName(
  ...
  if (result) {
    console.log(`[Successfully] create the Azure resource group with name: ${name}.`);
  } else {
    console.error(`[Failed] create the Azure resource group with name: ${name}.`);
  }
  return result;
}
...
export async function cleanTeamsApp(appName: string) {
  try {
    ...
    const installationId = await cleanService.getInstalledTeamsAppId(teamsUserId, appName);
...
export async function cleanAppStudio(appName: string) {
  try {
      ...
      for (const app of appStudioAppList) {
        if (app?.displayName?.startsWith(appName)) {
          ...
          try {
            ...
            console.log(`Failed to delete Teams App ${app?.displayName} in App Studio`);
          }
          break;
        }
      }
    ...
    console.log(`Failed to get apps in app studio, error message: ${e.message}`);
  }
}
...
export async function cleanUpStagedPublishApp(appId: string) {
  try {
    ...
    console.log(`Failed to get apps in admin portal, error message: ${e.message}`);
  }
}
