...
export async function initPage(
  ...
  await Promise.all([
    page.goto(`https://teams.microsoft.com/_#/l/app/${teamsAppId}?installAppPackage=true`),
  ...
  await RetryHandler.retry(async () => {
    ...
    // next
    await Promise.all([page.click("input.button[type='submit']"), page.waitForNavigation()]);
  });
  ...
  await RetryHandler.retry(async (retries: number) => {
    ...
    const frameElementHandle = await page.waitForSelector("iframe.embedded-page-content");
    ...
    if (options?.dashboardFlag) {
      ...
      if (popup && !popup?.isClosed()) {
        // input password
        console.log(`fill in password`);
        await popup.fill("input.input[type='password'][name='passwd']", password);
        // sign in
        await Promise.all([
    ...
    try {
      try {
        ...
        try {
          await page?.waitForSelector(".ts-messages-header span:has-text('About')");
        } catch (error) {
          try {
            await page?.waitForSelector(".team-information span:has-text('Chat')");
          } catch (error) {
            await page?.waitForSelector(".ts-messages-header span:has-text('Chat')");
          }
        }
      }
  ...
  try {
    ...
    await RetryHandler.retry(async (retries: number) => {
      ...
      if (options?.type === "meeting") {
        ...
        const frame = await frameElementHandle?.contentFrame();
        try {
          await frame?.waitForSelector(`h1:has-text('Add ${options?.teamsAppName} to a team')`);
        } catch (error) {
          await frame?.waitForSelector(`h1:has-text('Add ${options?.teamsAppName} to a meeting')`);
        }
      ...
      try {
        ...
        try {
          ...
          const searchBtn = await frame?.waitForSelector("div.ui-dropdown__toggle-indicator");
        ...
        const setUpBtn = await frame?.waitForSelector('button span:has-text("Set up a tab")');
      ...
      {
        const frameElementHandle = await page.waitForSelector("iframe.embedded-iframe");
        const frame = await frameElementHandle?.contentFrame();
...
export async function validateOneProducitvity(page: Page, options?: { displayName?: string }) {
  try {
...
export async function validateReactTab(page: Page, displayName: string, includeFunction?: boolean) {
  try {
    const frameElementHandle = await page.waitForSelector("iframe.embedded-iframe");
    const frame = await frameElementHandle?.contentFrame();
    if (includeFunction) {
...
export async function validateReactOutlookTab(
  ...
  try {
    await page.waitForTimeout(Timeout.longTimeWait);
    const frameElementHandle = await page.waitForSelector('iframe[data-tid="app-host-iframe"]');
    const frame = await frameElementHandle?.contentFrame();
    if (includeFunction) {
      await RetryHandler.retry(async () => {
        console.log("Before popup");
...
export async function validateBasicTab(page: Page, content = "Hello, World", hubState = "Teams") {
  try {
    const frameElementHandle = await page.waitForSelector("iframe.embedded-iframe");
    const frame = await frameElementHandle?.contentFrame();
...
export async function validateEchoBot(
  ...
  try {
    ...
    await RetryHandler.retry(async () => {
      ...
      await frame?.fill('div.ck-content[role="textbox"]', options?.botCommand || "helloWorld");
  ...
  try {
    ...
    if (options?.botCommand === "show") {
      try {
        ...
        console.log(`[Command "${options?.botCommand}" not executed successfully] ${e.message}`);
      }
      await RetryHandler.retry(async () => {
        // wait for alert message to show
        const btn = await frame?.waitForSelector(`div.ui-box button:has-text("Continue")`);
        await btn?.click();
      ...
      await RetryHandler.retry(async () => {
        console.log("sending message ", options?.botCommand);
        await executeBotSuggestionCommand(page, frame, options?.botCommand || "welcome");
        await frame?.click('button[name="send"]');
        await frame?.waitForSelector(`p:has-text("${options?.expected || ValidationContent.Bot}")`);
        console.log("verify bot successfully!!!");
      }, 2);
      console.log(`${options?.expected}`);
    }
...
export async function validateNpm(page: Page, options?: { npmName?: string }) {
  try {
    ...
    try {
      const targetItem = await frame?.waitForSelector(`span:has-text("${options?.npmName}")`);
...
export async function validateDeeplinking(page: Page, displayName: string) {
  try {
    ...
    await page.waitForTimeout(Timeout.shortTimeLoading);
    try {
      const detailsBtn = await page?.waitForSelector('li div a span:has-text("Details Tab")');
      await RetryHandler.retry(async () => {
    ...
    try {
      ...
      await RetryHandler.retry(async () => {
        ...
        const frameElementHandle = await page.waitForSelector("iframe.embedded-page-content");
        const frame = await frameElementHandle?.contentFrame();
        await frame?.waitForSelector('h1.center:has-text("Chat functionality")');
    ...
    try {
      const shareBtn = await page?.waitForSelector('li div a span:has-text("Generate Share URL")');
      await RetryHandler.retry(async () => {
        ...
        const closeBtn = await frame?.waitForSelector("dev.close-container button.icons-close");
...
export async function validateQueryOrg(page: Page, options?: { displayName?: string }) {
  try {
    ...
    const inputBar = await frame?.waitForSelector("div.ui-popup__content input.ui-box");
    await inputBar?.fill(options?.displayName || "");
    await page.waitForTimeout(Timeout.shortTimeLoading);
    const loginBtn = await frame?.waitForSelector('div.ui-popup__content a:has-text("sign in")');
...
export async function validateShareNow(page: Page) {
  try {
    ...
    const startBtn = await frame?.waitForSelector('span:has-text("Suggest content")');
    await startBtn?.click();
    await page.waitForTimeout(Timeout.shortTimeLoading);
    // select content type
    console.log("select content type");
    const popupModal = await frame?.waitForSelector(".ui-dialog .dialog-provider-wrapper");
    const typeSelector = await popupModal?.waitForSelector(
      'button:has-text("Select content type")'
    );
    await typeSelector?.click();
    await page.waitForTimeout(Timeout.shortTimeLoading);
    const item = await popupModal?.waitForSelector('ul li:has-text("Article / blog")');
    await item?.click();
    await page.waitForTimeout(Timeout.shortTimeLoading);
    // fill in title
  ...
  try {
    ...
    await RetryHandler.retry(async () => {
      try {
        ...
        console.log(`[Command "welcome" not executed successfully] ${e.message}`);
      }
      try {
        await frame?.waitForSelector(
...
export async function validateTodoList(page: Page, options?: { displayName?: string }) {
  try {
    ...
    try {
      ...
      await RetryHandler.retry(async () => {
        ...
        const addBtn = await frame?.waitForSelector('button:has-text("Add task")');
        await addBtn?.click();
...
async function executeBotSuggestionCommand(page: Page, frame: null | Frame, command: string) {
  try {
    ...
    try {
      ...
      console.log(`[Command ${command} not executed successfully] ${e.message}`);
    }
  }
}
...
export async function validateSpfx(page: Page, options?: { displayName?: string }) {
  try {
    const frameElementHandle = await page.waitForSelector("iframe.embedded-page-content");
    const frame = await frameElementHandle?.contentFrame();
...
export async function validateContact(page: Page, options?: { displayName?: string }) {
  try {
...
export async function validateGraphConnector(page: Page, options?: { displayName?: string }) {
  try {
    console.log("start to verify contact");
    await page.waitForTimeout(Timeout.shortTimeLoading);
    const frameElementHandle = await page.waitForSelector("iframe.embedded-page-content");
...
export async function validateNotificationTimeBot(page: Page) {
  try {
    ...
    await frame?.waitForSelector("div.ui-box");
    await RetryHandler.retry(async () => {
      await frame?.waitForSelector(`p:has-text("This is a sample time-triggered notification")`);
    ...
    await RetryHandler.retry(async () => {
      try {
        ...
        if (options?.env === "dev") {
          ...
          url = devEnv.obj["BOT_FUNCTION_ENDPOINT"] + "/api/default-notification";
        } else {
...
export async function addPerson(frame: Frame | null, displayName: string): Promise<void> {
  ...
  await item?.click();
  await frame?.waitForSelector(`div.table-area div.line1:has-text("${displayName}")`);
}

export async function delPerson(frame: Frame | null, displayName: string): Promise<void> {
  console.log(`delete person: ${displayName}`);
  await frame?.waitForSelector(`li div.details.small div:has-text("${displayName}")`);
