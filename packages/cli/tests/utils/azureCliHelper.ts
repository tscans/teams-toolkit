...
export class AzSqlHelper {
  ...
  constructor(
    ...
    this.sqlServerName = sqlServerName || `testsql${Math.floor(Math.random() * 100000)}`;
    this.sqlDatabaseName = sqlDatabaseName || `testdb${Math.floor(Math.random() * 100000)}`;
    ...
    this.storageAccountName = `teststorage${Math.floor(Math.random() * 100000)}`;
    this.storageContainerName = `testcontainer${Math.floor(Math.random() * 100000)}`;
  ...
  public async createSql() {
    ...
    const { success: sqlEndpointSuccess, sqlServerEndpoint } = await this.getSqlEndpoint();
  ...
  static async listResourceGroup(prefix: string): Promise<{ success: boolean; stdout: string[] }> {
