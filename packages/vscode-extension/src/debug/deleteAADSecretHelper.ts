...
export async function deleteAADSecret() {
  try {
    ...
    if (accountInfo !== undefined) {
      ...
      // aadClient.interceptors.request.use(async (config) => {
      ...
      for (const key in deleteMap) {
        ...
        if (res.status != 204) {
          // VsCodeLogInstance.warning("Failed to delete secret for AAD app: " + key + ", status code: " + res.status.toString() + ", error message: " + res.statusText);
        } else {
    ...
    // VsCodeLogInstance.warning("Failed to delete AAD secret: " + error.message);
  }
}
